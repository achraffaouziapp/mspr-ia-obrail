...
| RGPD / gouvernance        | Maîtrise des données, sécurité, hébergement et conformité          |
...
| ---------------------------- | ---------------------------------------------------------------------- |
| Plateforme complète          | Azure ML couvre une grande partie du cycle de vie machine learning     |
| Bon niveau MLOps             | Suivi des expériences, versionnement, déploiement et monitoring        |
...
| Compatible avec scikit-learn | Un modèle Python existant peut être intégré dans l’écosystème Azure ML |
...
| Limite                     | Explication                                                                  |
| -------------------------- | ---------------------------------------------------------------------------- |
| Coût variable              | Le coût dépend des ressources cloud utilisées                                |
...
| Gouvernance à encadrer     | Il faut définir précisément l’hébergement, les droits d’accès et la sécurité |
...
* un besoin de déploiement scalable.
...
| Adapté au tabulaire    | Compatible avec des tâches de classification et régression tabulaires |
...
| Limite                     | Explication                                                  |
| -------------------------- | ------------------------------------------------------------ |
| Coût potentiellement élevé | Coûts liés au calcul, au stockage et aux endpoints           |
...
| Mise en place plus longue  | Nécessite de configurer les services, droits IAM et stockage |
...
| Scalabilité              | Adapté à une montée en charge cloud                     |
...
| Surdimensionné pour le prototype     | Trop complet pour le besoin actuel                                   |
...
Pour la version actuelle, l’approche interne reste plus adaptée car elle donne plus de contrôle sur :

* la création de la cible ;
* les règles métier ;
* les variables utilisées ;
* les scripts de ré-entraînement ;
* l’API `/predict`.
...
| Open source                     | Approche plus ouverte que certains grands clouds |
| Rapide pour le prototypage      | Permet de tester rapidement une approche AutoML  |
| Bonne intégration Hugging Face  | Compatible avec l’écosystème Hugging Face        |
| Moins lourd qu’un cloud complet | Plus simple que SageMaker, Azure ML ou Vertex AI |
| Accessible                      | Intéressant pour tester rapidement une idée      |
...
| Intégration API à prévoir     | Il faudrait adapter le modèle au système FastAPI existant |
...
Cependant, pour ObRail, le besoin principal est de maîtriser :

* les règles métier ;
* la donnée ferroviaire ;
* la cible construite ;
* les transformations ;
* l’intégration API.

Le modèle interne est donc plus adapté pour cette première version.
...
| Suivi des pipelines   | Possibilité de classer et comparer plusieurs pipelines candidats |
...
| Complexité d’écosystème          | Nécessite une prise en main IBM Cloud ou watsonx             |
| Surdimensionné pour le prototype | Trop complet pour une V1 académique                          |
| Dépendance fournisseur           | Dépendance à l’écosystème IBM                                |
| Déploiement local moins direct   | Moins simple qu’un modèle scikit-learn sauvegardé localement |
...
| Tests automatisés      | Contrôle du dataset, du modèle et de la prédiction |
...
| Déploiement à organiser | L’API fonctionne localement mais n’est pas encore déployée en production |
...
L’usage d’un cloud public nécessiterait une analyse plus complète sur :
...
7. absence de récupération automatique des variables par `route_id`.
...
tester automatiquement les éléments clés
```

La solution retenue est donc :

```text
Pipeline IA interne basé sur Python, scikit-learn, joblib, FastAPI et pytest.
```
